...
Manifest 显式声明同时支持 `desktop` 和 `server`，两者是同一实例的互斥运行位置，不是同时运行的两个 Backend。Desktop 模式只在客户端运行期间在线；迁移到 Server 后由 Server 独立维持长连接，适合 7×24 运行。两个模式分别声明实际使用的 `moss.agent/v1` 协议。
