...
export function openIMConversationScope(userId: string) {
  return `${OPENIM_CONVERSATION_PREFIX}${encodeURIComponent(requiredIdentifier(userId, "OpenIM user id"))}`;
}

export function openIMDefaultConversationId(userId: string) {
  return `${openIMConversationScope(userId)}/*`;
}

export function openIMDirectConversationId(userId: string, peerUserId: string) {
  return `${openIMConversationScope(userId)}/direct:${encodeURIComponent(requiredIdentifier(peerUserId, "OpenIM peer user id"))}`;
}

export function parseOpenIMDirectConversationId(value: unknown) {
  const normalized = typeof value === "string" ? value.trim() : "";
...
export function openIMDefaultConversationIdFor(value: unknown) {
  const normalized = typeof value === "string" ? value.trim() : "";
